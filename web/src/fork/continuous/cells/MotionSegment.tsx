/**
 * fork/continuous/cells — COPIED from upstream `web/src/components/timeline/MotionSegment.tsx`
 * upstream tag: 0.17.1 (fork base 0.17.1-troy.10)   sha256 at copy: 9fa0a58c5de2f92a76ea3a0ac34a09e0c64c06f12883eb690410c4887463fbe9
 *
 * Why copied rather than imported (handover §7.4 / D8): the fork feeds these cells a
 * PRE-SLICED `events` array (useSegmentEventIndex) instead of the whole review list. That
 * is only correct while every helper here selects events with the overlap predicate
 *   time >= segStart(e.start_time) && time < segEnd(e.end_time)
 * and probes at most ±1 segment (shouldShowRoundedCorners). Upstream widening that probe
 * would make an *imported* cell silently wrong; a copy pins the semantics we depend on.
 * `fork/continuous/UPSTREAM_SHAS` + `__tests__/upstreamDrift.test.ts` turn drift into a
 * loud failure at build time. Do not edit upstream's comments below.
 */
